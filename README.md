# Project Files Overview
This repository contains various images related to the project, including screenshots of the database, API, editor interface, and other visual assets.

## File List

| File Name   | Description                     | Last Commit Message       | Last Commit Date |
|------------|---------------------------------|--------------------------|----------------|
| `DB.PNG`    | Screenshot of the database view | Add files via upload     | now             |
| `api.PNG`   | Screenshot of the API interface | Add files via upload     | now             |
| `editor.PNG`| Screenshot of the editor tool   | Add files via upload     | now             |
| `main.PNG`  | Main interface screenshot       | Add files via upload     | now             |

![DB Screenshot](ScreenShots/main.PNG)
![API Screenshot](ScreenShots/DB.PNG)
![DB Screenshot](ScreenShots/api.PNG)
![API Screenshot](ScreenShots/editor.PNG)

